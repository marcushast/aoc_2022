object Day04 {
    ...
    fun process1(data: String): List<Pair<IntRange, IntRange>> =
        data.lines().map {
            ...
            Pair(stringToRange(pair[0]), stringToRange(pair[1]))
        }

    private fun stringToRange(rangeString: String): IntRange {
        val index = rangeString.split("-")
    ...
    fun work1(data: List<Pair<IntRange, IntRange>>) {
        val containedPairs = data.filter {
            ...
            combined.size == Math.max(first.size, second.size)
        }
    ...
    fun work2(data: List<Pair<IntRange, IntRange>>) {
        val containedPairs = data.filter {
            ...
            combined.size < first.size + second.size
        }

        println("Number of pairs with any overlap: ${containedPairs.size}")
    }

    fun task1_test() {
        work1(process1(data1))
    }

    fun task1_real() {
        work1(process1(data2))
    }

    fun task2_test() {
        work2(process1(data1))
    }

    fun task2_real() {
        work2(process1(data2))
    }
